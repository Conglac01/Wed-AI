...
export function AuthLayout() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      >
        <div className="mx-auto flex w-full max-w-[1400px] items-center justify-between px-3 sm:px-6">
          {/* Logo */}
          <Link
            to="/"
            className="flex shrink-0 items-center gap-2 text-sm font-bold tracking-wide text-gray-900"
            style={{ fontSize: "14px" }}
          >
            {/* Gear / cog icon */}
            <svg className="h-5 w-5 flex-shrink-0 sm:h-6 sm:w-6" style={{ color: "#1E5FD4" }} fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
            ...
            <span className="hidden sm:inline">VIECCONNECT IT JOBS</span>
            <span className="text-xs font-bold text-gray-900 sm:hidden">VIECCONNECT</span>
          </Link>
          ...
          <nav className="flex items-center gap-2 text-sm sm:gap-3">
            {isLoading ? (
              ...
              <>
                <Link
                  to="/profile"
                  className="flex items-center gap-2 rounded-lg px-2 py-2 text-gray-600 hover:bg-gray-100 sm:px-3"
                >
                  {/* User icon */}
                ...
                <button
                  onClick={() => logout()}
                  className="rounded-lg border px-3 py-2 text-xs font-medium text-gray-700 transition-colors hover:bg-gray-50 sm:px-4 sm:text-sm"
                  style={{ borderColor: "#EBEBEB" }}
                >
                  Đăng xuất
                </button>
              ...
              <>
                <Link
                  to="/login"
                  className="rounded-lg border px-3 py-2 text-xs font-medium transition-colors hover:bg-gray-50 sm:px-5 sm:text-sm"
                ...
                <Link
                  to="/register"
                  className="rounded-lg px-3 py-2 text-xs font-medium text-white transition-colors sm:px-5 sm:text-sm"
